-- Drop the old event-level unique constraint
ALTER TABLE attendance_records DROP CONSTRAINT IF EXISTS unique_user_event_attendance;

-- Add a session-level unique constraint so a user can check in once per session
ALTER TABLE attendance_records ADD CONSTRAINT unique_user_session_attendance UNIQUE(user_id, session_id);

-- Update the check-in function to verify uniqueness per session rather than per event
...
BEGIN
  ...
  -- Check if already marked for THIS session (not the entire event)
  IF EXISTS (
    SELECT 1
